...
for i in range(2019,2022) :
    data[data['rentree_scolaire'] == i].hist(column = 'nombre_total_eleves', bins = int(data[data['rentree_scolaire'] == 2021]['nombre_total_eleves'].max()), grid=False, figsize=(12,8), color='#86bf91', zorder=2, rwidth=0.9)
    print(data[data['rentree_scolaire'] == i]['nombre_total_eleves'].sum())
plt.show()
data = data[data['rentree_scolaire'] == 2021]
...
regions_data2 = data.drop(['rentree_scolaire', 'academie', 'departement', 'commune', 'numero_ecole', 'denomination_principale', 'patronyme', 'secteur', 'rep', 'rep_plus', 'tri', 'code_postal'], axis=1)
regions_data2["nombre d'élèves moyen par classe"] = regions_data2["nombre_total_eleves"] / regions_data2["nombre_total_classes"]
regions_data2 = regions_data2.groupby(['region_academique']).agg({'region_academique' : 'first', "nombre d'élèves moyen par classe" : 'mean'})
regions_data2.reset_index(drop = True, inplace=True)

departements_data = data.drop(['rentree_scolaire', 'academie', 'region_academique', 'commune', 'numero_ecole', 'denomination_principale', 'patronyme', 'secteur', 'rep', 'rep_plus', 'tri', 'code_postal'], axis=1)
departements_data = departements_data.groupby(['departement']).agg({'departement' : 'first','nombre_total_eleves' : 'mean'})
departements_data.reset_index(drop = True, inplace=True)

def RenameRegion(regions_data):
...
fill_opacity=0.9,
line_opacity=0.2,
legend_name="Median nombre d'élève par école",
highlight=True,
show=False,
overlay=True,
name="Région",
...
region = folium.features.GeoJson(
    ...
    tooltip=folium.features.GeoJsonTooltip(
        ...
        aliases=['Région :', "nombre d'eleves moyen par école (région)"],
